...
def prange(obs, nav, opt, var):
    ...
    if opt.ionoopt == prl.IONOOPT_IFLC:  # dual-frequency
        if sys == prl.SYS_GPS or sys == prl.SYS_QZS:  # L1-L2, G1-G2
            ...
            gamma = (
                (prl.FREQ1_CMP if obs.code[0] == prl.CODE_L2I else prl.FREQ1)
                / prl.FREQ2_CMP
            ) ** 2
            b1 = (
                gettgd(sat, nav, 0)
                if obs.code[0] == prl.CODE_L2I
                else (
                    gettgd(sat, nav, 2)
                    if obs.code[0] == prl.CODE_L1P
                    else gettgd(sat, nav, 2) + gettgd(sat, nav, 4)
                )
            )  # TGD_B1I / TGD_B1Cp / TGD_B1Cp+ISC_B1Cd
        ...
        if sys == prl.SYS_GPS or sys == prl.SYS_QZS:  # L1
            ...
            b1 = (
                gettgd(sat, nav, 0)
                if prl.getseleph(prl.SYS_GAL)
                else gettgd(sat, nav, 1)
            )  # BGD_E1E5a / BGD_E1E5b
            return P1 - b1
...
def get_rtklib_pnt(obs, nav, prcopt, mode):
    if mode == "SPP":
        ...
        if (
            msg.ptr
            and "chi-square error" not in msg.ptr
            and "gdop error" not in msg.ptr
        ):
            return sol, False
        else:
            return sol, True
    if mode == "DGNSS":
        ...
        if (
            msg
            and "ambiguity validation failed" not in msg
            and "chi-square error" not in msg
            and "slip detected forward" not in msg
            and "slip detected half-cyc" not in msg
            and "no double-differenced residual" not in msg
        ):
...
def get_dgnss_pos(obs, sta, nav):
    ...
    for i in range(len(com)):
        ...
        if i == no:
            ...
            continue
        r21 = np.linalg.norm(
            np.array(rs[no * 6 + 0 : no * 6 + 3]) - np.array(sta.pos)
        ).astype("float64")
        r22 = np.linalg.norm(
            np.array(rs[i * 6 + 0 : i * 6 + 3]) - np.array(sta.pos)
        ).astype("float64")
...
def DD_H_matrix(satpos, p00, ms, sys_map):
    ...
    for i in range(n):
        ...
        Ht.append(
            np.array(
                [
                    -(p[0] - p00[0]) / r - (-(p0[0] - p00[0])) / r0,
                    -(p[1] - p00[1]) / r - (-(p0[1] - p00[1])) / r0,
                    -(p[2] - p00[2]) / r - (-((p0[2] - p00[2])) / r0),
                ]
            )
        )
        Rt.append(r - r0)
...
def get_wls_pnt_pos(o, nav, exsatids=[], SNR=0, EL=0, RESD=10000):
    ...
    while np.linalg.norm(dp) > 0.0001 and iii < maxiter:
        H, R, azel, ex, sysinfo, syscount, vels, vions, vtrps = H_matrix_prl(
            rs, p, dts, o.data[0].time, nav, sats, exclude
        )
...
def get_nlos_wls_pnt_pos(o, nav, nlos, exsatids=[]):
    ...
    b = np.zeros((o.n - len(exclude), 1))
    while np.linalg.norm(dp) > 0.0001 and iii < maxiter:
        H, R, azel, ex, sysinfo, syscount, vels, vions, vtrps = H_matrix_prl_nlos(
            rs, p, dts, o.data[0].time, nav, sats, nlos, exclude
        )
        inc = set(range(o.n - len(noeph))) - set(ex)
        if len(inc) < 4:
            iii += 1
...
if torch_enable:
    ...
    def H_matrix_prl_torch(satpos, rp, dts, time, nav, sats, exclude=[]):
        ...
        for i in range(n):
            ...
            r = (
                torch.norm(sp - rp[:3])
                + prl.OMGE * (sp[0] * rp[1] - sp[1] * rp[0]) / prl.CLIGHT
            )
            azel_tmp = prl.Arr1Ddouble(2)
            prl.satazel(pos, e, azel_tmp)
            azel[i * 2] = azel_tmp[0]
            ...
            if azel_tmp[1] < 0:
                ...
                continue
            prl.ionocorr(
                time, nav, sats[i], pos, azel_tmp, prl.IONOOPT_BRDC, dion, vion
            )
            prl.tropcorr(time, nav, pos, azel_tmp, prl.TROPOPT_SAAS, dtrp, vtrp)
            vions.append(vion.ptr)
            vtrps.append(vtrp.ptr)
                ...
                Ht.append(
                    torch.hstack(
                        [
                            ...
                            torch.tensor([1, 0, 0, 0], dtype=torch.float64).to("cuda"),
                        ]
                    )
                )
                Rt.append(r + rp[3] - prl.CLIGHT * dts[i * 2] + dtrp.ptr + dion.ptr)
                ...
                Ht.append(
                    torch.hstack(
                        [
                            ...
                            torch.tensor([0, 1, 0, 0], dtype=torch.float64).to("cuda"),
                        ]
                    )
                )
                Rt.append(r + rp[4] - prl.CLIGHT * dts[i * 2] + dtrp.ptr + dion.ptr)
                ...
                Ht.append(
                    torch.hstack(
                        [
                            ...
                            torch.tensor([0, 0, 1, 0], dtype=torch.float64).to("cuda"),
                        ]
                    )
                )
                Rt.append(r + rp[5] - prl.CLIGHT * dts[i * 2] + dtrp.ptr + dion.ptr)
                ...
                Ht.append(
                    torch.hstack(
                        [
                            ...
                            torch.tensor([0, 0, 0, 1], dtype=torch.float64).to("cuda"),
                        ]
                    )
                )
                Rt.append(r + rp[6] - prl.CLIGHT * dts[i * 2] + dtrp.ptr + dion.ptr)
    ...
    def get_ls_pnt_pos_torch(o, nav, w=None, b=None, p_init=None, exsatids=[]):
        ...
        while torch.norm(dp) > 0.0001 and iii < maxiter:
            ...
            H, R, azel, ex, sysinfo, syscount, vels, vions, vtrps = H_matrix_prl_torch(
                rs, p, dts, o.data[0].time, nav, sats, exclude
            )
            inc = set(range(o.n - len(noeph))) - set(ex)
            if len(inc) < 4:
        ...
        H, R, azel, ex, sysinfo, syscount, vels, vions, vtrps = H_matrix_prl(
            rs, p.detach().cpu().numpy(), dts, o.data[0].time, nav, sats, exclude
        )
        return {
            "status": True,
            "pos": p,
...
def goGPSw(S, el):
    ...
    def w(S, theta):
        if S < s_1:
            return (1 / np.sin(theta) ** 2) * (
                10 ** k1(S) * ((A / 10 ** k1(s_0) - 1) * k2(S) + 1)
            )
        else:
            return 1
